...
| 管理・承認 | **front 管理画面 `/reserve/admin`（bearer トークン `ADMIN_TOKENS`）**＋ LINE/Discord 通知のボタン（front `/reserve/decision`）からも承認 |
...
- GAS: 各プロジェクトの **Script Properties** に LINE トークン・カレンダーID・管理トークン（`ADMIN_TOKENS`・front 管理画面 bearer 認証用）・HMAC secret を環境別に格納。
- `gas/` をリポジトリで版管理。**clasp** のターゲット切替（`.clasp.dev.json` / `.clasp.prod.json`）で dev/prod に push/deploy。デプロイは**公開ウェブアプリ①の単一デプロイ**（`executeAs=USER_DEPLOYING` / `access=ANYONE_ANONYMOUS`）。
...
- 手順の詳細は [`SETUP.md`](./SETUP.md) I を参照。

---

## 作るもの（概要）

1. **GAS バックエンド**（`gas/`）: `getAvailability` / `createBooking` / `cancelBooking` / `changeBooking` / `getSlotConfig` / `setSlotConfig`、承認/辞退（`decide`）・顧客メッセージ（`messageInfo`/`messageSend`）・管理系（bearer `ADMIN_TOKENS` 保護）の doPost アクション、HMAC 署名。**単一の公開ウェブアプリ①**（`doGet` は availability/booking のみ、状態変更はすべて doPost）。
2. **フロント**（Astro 静的）: `src/pages/reserve/index.astro`、`src/pages/reserve/manage.astro`、`src/pages/reserve/admin.astro`（管理）、`src/pages/reserve/decision.astro`（承認/辞退）、`src/pages/reserve/message.astro`（顧客メッセージ）、`src/data/menu.js`、`src/data/config.js`。
3. **管理画面**（front `/reserve/admin`）: 空き枠開閉＋承認/辞退＋一斉送信/臨時営業休業など。**bearer トークン `ADMIN_TOKENS`** で認可（Google ログイン不要＝管理デプロイ②不要）。
