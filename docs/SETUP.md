...
| `PUBLIC_EXEC_URL` | 公開API（①デプロイ）の `/exec` URL。店通知の承認/辞退リンクの基底に使う。`ScriptApp.getService().getUrl()` は複数デプロイ環境で別デプロイのURLを返し不安定なため固定する。①の公開デプロイ作成後にその `/exec` をコピーして登録（未設定なら getUrl にフォールバック）|
