...
5. **dev/prod の区別**: 同じ LINE に届くため、dev の GAS には Script Property `ENV_LABEL` に `【開発】` を設定し、店向け通知の先頭に付けて見分ける（prod は未設定＝空）。承認/辞退リンクは各 GAS の URL を指すので、dev リンクは dev、prod リンクは prod に作用し誤承認はしない。
...
| `ENV_LABEL` | 店向け LINE 通知の先頭ラベル。**dev のみ** `【開発】` を設定（prod は未設定＝空）。LINE Messaging API が dev/prod 共有のため通知を区別する |
...
- [×] `FRONT_BASE_URL`
- [ ] `ENV_LABEL`（dev のみ `【開発】`。LINE 共有チャネルの通知区別用）
