...
| `ADMIN_EMAILS` | 管理画面を使える Google アカウントのメール（カンマ区切り）。管理者の追加/運用は [`../gas/README.md`](../gas/README.md)「管理者ユーザーの登録・管理」参照 |
...
| `PUBLIC_EXEC_URL` | 公開API（①デプロイ）の `/exec` URL。**承認/辞退リンクは `FRONT_BASE_URL` 経由に移行済み**のため現在は旧 GET 決定ページ（`renderDecisionPage_`・後方互換）と `ADMIN_EXEC_URL` 未設定時のフォールバック・`diag` ログでのみ使用。`ScriptApp.getService().getUrl()` は複数デプロイ環境で別デプロイのURLを返し不安定なため固定する。①の公開デプロイ作成後にその `/exec` をコピーして登録（未設定なら getUrl にフォールバック）|
| `ADMIN_EXEC_URL` | 管理デプロイ（②・executeAs=アクセスユーザー/要ログイン）の `/exec` URL。予約客への任意メッセージ送信リンク（`?action=message`）の基底に使う（カレンダーイベント説明欄のメッセージリンクがこのURLを指す）。②管理デプロイ作成後にその `/exec`（`?action=admin` を除いた部分）を登録。**未設定時は `PUBLIC_EXEC_URL`（①）へフォールバックするが、その場合 `requireAdmin_` が `getActiveUser()` を取れず forbidden になる**（＝安全側・誤送信は起きない） |
...
4. 管理画面用は **別デプロイ**にし、実行ユーザーを **「ウェブアプリにアクセスしているユーザー」**、アクセスを用途に応じて「自分のみ」/「組織内の全員」/「Googleアカウントを持つ全員」に設定する。**「自分（オーナー）として実行」にすると別ドメイン（gmail等）の管理者で `getActiveUser()` が空になり弾かれる**点に注意。手順・運用パターン・費用は [`../gas/README.md`](../gas/README.md)「管理者ユーザーの登録・管理」を参照。
...
3. push 送信履歴は顧客台帳スプレッドシートに **「送信ログ」シートが自動作成**され記録される（宛先はマスク）。当月送信数は管理画面（`?action=admin`）「LINE無料枠（当月）」でも確認できる。
...
- [×] `FRONT_BASE_URL`
- [ ] `PUBLIC_EXEC_URL`（公開API①の `/exec`。承認/辞退リンク基底の固定用。デプロイ作成後に登録）
