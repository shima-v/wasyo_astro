...
| `MONTHLY_FREE_QUOTA` | （任意）LINE Messaging API 無料枠の月上限。未設定は既定 `200`。`checkQuota_` の80%警告判定・管理画面「LINE無料枠（当月）」表示に使う |
...
2. 以下を**日次**で登録（実行時間帯は任意。リマインド/フォローは午前中が目安）:
   - `sendReminders_` … 翌日の確定予約へ前日リマインド（`reminded` タグで多重防止）
   - `sendFollowUps_` … 前日に終了した確定予約へお礼/再来店（`followedUp` タグ）
   - `checkQuota_` … 当月送信数を確認し80%超でオーナー警告（`QUOTA_WARNED_YYYYMM` で月内多重防止）
