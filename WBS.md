...
| Phase 1 | GAS バックエンド | 実装・デプロイ完了（残=👤 初回認可/admin確認） | 20 / 21 |
| Phase 2 | フロント（予約UI） | 実装ほぼ完了（残=LINE Login任意） | 13 / 14 |
| Phase 3 | 既存サイト統合・環境切替 | ほぼ完了（残=👤 repo secret 登録） | 5 / 6 |
| Phase 4 | 検証・リリース | 進行中（API層 e2e 検証済・残=承認/UI/リリース） | 3 / 8 |
...
- [~] 1.6.2 dev 単体動作確認：公開API `?action=availability` が `ok:true` で空き枠を返し、**営業ルール（翌日以降・日祝休・第3土休/第4土営業・30分刻み10:00-19:30）も実カレンダーで検証済み**🤖。初回認可も完了済み。👤残= `?action=admin` をブラウザでオーナーログイン確認
...
- [~] 4.1.3 e2e: 仮予約→devカレンダー【仮】→店LINE通知→承認→【確定】＋確定通知。**仮予約までを curl で確認済み**🤖（createBooking `ok:true`・トークン発行）。👤残=店LINE/客メール受信確認＋承認で【確定】化
- [x] 4.1.4 新規/常連判定・初回料金の自動適用 🤖（新規=`isFirstTime:true`、初回40分・¥3,300 が自動適用）
- [~] 4.1.5 取消/変更がカレンダー・空き枠に反映：**変更を curl で確認済み**🤖（15:00→16:30・再承認のため pending 復帰）。取消は承認後にcurlで確認予定
