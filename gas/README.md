...
`CALENDAR_ID` / `LEDGER_SHEET_ID` / `LINE_CHANNEL_ACCESS_TOKEN` / `LINE_OWNER_USER_ID` / `ADMIN_EMAILS` / `HMAC_SECRET` / `FRONT_BASE_URL` / `PUBLIC_EXEC_URL`（任意: `LINE_LOGIN_*` / `SLOT_CONFIG` / `ENV_LABEL`）。
...
理由（実際に踏んだ不具合）: 承認/辞退を「GETで即実行」する実装にしていたところ、**LINE のリンクプレビュー用クローラが通知内のURLを自動取得し、辞退リンクのGETを実行 → 仮予約イベントが作成後数十秒で勝手に削除**された（承認リンクも踏まれ一瞬 `confirmed` 化→削除、という挙動も観測）。メール/チャットのリンク先読みは一般的なので、**状態変更は必ず POST 相当（ここでは `google.script.run`）に分離**する。
