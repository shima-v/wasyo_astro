...
`CALENDAR_ID` / `LEDGER_SHEET_ID` / `LINE_CHANNEL_ACCESS_TOKEN` / `LINE_OWNER_USER_ID` / `ADMIN_EMAILS` / `HMAC_SECRET` / `FRONT_BASE_URL`（任意: `LINE_LOGIN_*` / `SLOT_CONFIG`）。
詳細と取得方法は [`../docs/SETUP.md`](../docs/SETUP.md) F を参照。`SLOT_CONFIG` は管理パネルから保存されるため手動設定は不要。

## 動作確認（dev）

1. `clasp push` → ② の管理デプロイ URL に `?action=admin` を付けて開く → 自分のGoogleアカウントで承認 → 管理パネル表示。
